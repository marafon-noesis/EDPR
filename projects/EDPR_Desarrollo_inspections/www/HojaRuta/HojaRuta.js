...
FS.HojaRuta = {
    ...
    onLoad: function (entityForm) {
        ...
        esBOQ = false;
        FS.HojaRuta.esOrdenBOQ(entityForm);
        if (HojaRuta.isNew) {
            guardar = true;
            entityForm.entity.properties.atos_finalizada = true;
        } else {
            guardar = false;
    ...
    onPostSave: function (entityForm) {
        ...
        if (entityForm.entity.properties.atos_ordendetrabajoid != null) {
            ...
            //var myButton = Resco.Controls.CurrentForm.getControl("myButtonId"); // replace "myButtonId" with the actual ID of your button
        ...
        /*var HRid = entityForm.entity.id;
        guardar=false;
        var entity = entityForm.entity;
        var self = this;
        var fetchXml =  "<fetch version='1.0'>" +
        "<entity name='atos_hojaderuta'>" +
        "<attribute name='atos_hojaderutaid' />" +
        "<attribute name='atos_almacenpordefecto' />" +
        "<filter type='and'>" +
        "<condition attribute='atos_hojaderutaid' operator='eq' value='" + entityForm.entity.properties.atos_hojaderutaid.id + "'/>" +
        "</filter>" +
        "</entity>" +
        "</fetch>";
    ...
    esOrdenBOQ: function (entityForm) {
        ...
        var fetchXml = "<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>" +
            ...
            "    </filter>" +
            "    <link-entity name='atos_clasedeactividad' from='atos_clasedeactividadid' to='atos_claseactividadpmid' link-type='inner' alias='ab'>" +
        ...
        MobileCRM.FetchXml.Fetch.executeFromXML(fetchXml,
            function (ordenDeTrabajo) {
                for (var i in ordenDeTrabajo) {
                    var codigo = ordenDeTrabajo[i][1];
                    if (codigo == "115" || codigo == "116") {
                        esBOQ = true;
                    }
    ...
    ClonarHojaRutaOT: function (entityForm) {
        ...
        try {
            ...
            var fetchXmlOrden = "<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>" +
                ...
                "    <attribute name='msdyn_name' />" +
                "    <attribute name='atos_claseactividadpmid' />" +
                "    <filter type='and'>" +
                "      <condition attribute='msdyn_workorderid' operator='eq' value='" + HojaRuta.properties.atos_ordendetrabajoid.id + "' />" +
                "    </filter>" +
                "    <link-entity name='atos_clasedeactividad' from='atos_clasedeactividadid' to='atos_claseactividadpmid' link-type='inner' alias='ac'>" +
                "		<attribute name='atos_aplicaboq' />" +
                "    </link-entity>" +
                "  </entity>" +
                "</fetch>";
            ...
            MobileCRM.FetchXml.Fetch.executeFromXML(fetchXmlOrden,
                function (resultOT) {
                    ...
                    if (resultOT[0][1] != 1) {
                        ...
                        FS.HojaRuta.crearDatosHojaRutaOT(entityForm, resultOT, entityForm.entity.properties.atos_hojaderutaid.id, HojaRutaPlantilla);
                        ...
                        if (esBOQ) {
                            ...
                            FS.HojaRuta.crearDatosHojaRutaOT(entityForm, resultOT, entityForm.entity.properties.atos_hojaderutaid.id, HojaRutaPlantilla);
                            //23374:AAC:2022-02-18:Quitar posibilidad de borrar operaciones al añadir hoja de ruta
                            //	popup.multiLine = true;
                            //	popup.show(
                            //		function (button) {
                            //			if (button == "Yes" || button == "Si") {
                            //				// obtengo las operaciones  de la OT
                            //				FS.HojaRuta.DesactivarOperacionesOT( entityForm,resultOT,HojaRutaPlantilla);
                            //			}
                            //			else {
                            //			   // creamos la hoja de ruta
                            //				FS.HojaRuta.crearDatosHojaRutaOT( entityForm,resultOT,entityForm.entity.properties.atos_hojaderutaid.id,HojaRutaPlantilla);
                            //			}
                            //			//23144:AAC:14/10/2021: Se puede tener hojas de ruta repetidas
    ...
    crearDatosHojaRutaOT: function (entityForm, orden, hojaRutaId, HojaRuta) {
                ...
                MobileCRM.FetchXml.Fetch.executeFromXML(fetchXmlUbicacion,
                    function (resultUbicacion) {
                        ...
                        if (resultUbicacion.length > 0) {
                            ...
                            FS.HojaRuta.obtenerAlmacenOTdeUbicacion(entityForm, orden, ubicacion, HojaRuta);
                            FS.HojaRuta.crearOperacionesRutaOT(entityForm, orden, hojaRutaId, ubicacion, tipoMovimiento, HojaRuta);
                        } else {
    ...
    obtenerAlmacenOTdeUbicacion: function (entityForm, orden, ubicacion, HojaRuta) {
        if (ubicacion == null || ubicacion[2] == null) {
            return;
        }
        var condicion = "";
        if (orden[0][6] == "True" && HojaRuta[1] == "True") {

            condicion += "		<condition attribute='atos_centrodecosteid' operator='eq' value='" + ubicacion[0].id + "'/>";
            condicion += "		<condition attribute='atos_centroid' operator='eq' value='" + ubicacion[2].id + "' />";
            condicion += "		<condition attribute='atos_indicadorboq' operator='eq' value='1'/>";

        } else {
            condicion += "      <condition attribute='atos_centroid' operator='eq'  uitype='atos_centrodecoste' value='" + ubicacion[2].id + "' />";
            condicion += "		<condition attribute='atos_indicadorboq' operator='ne' value='1'/>"
    ...
    crearOperacionesRutaOT: function (entityForm, orden, hojaRutaId, ubicacion, tipoMovimiento, HojaRuta) {
        var operacion = 0;
        ...
        var fetchXmlOperaciones = "<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>" +
            ...
            "    <order attribute='atos_numerodenodo' descending='false'/>" +
            "    <filter type='and'>" +
            "      <condition attribute='atos_hojaderutaid' operator='eq' value='" + hojaRutaId + "' />" +
        ...
        MobileCRM.FetchXml.Fetch.executeFromXML(fetchXmlOperaciones,
            function (resultOperaciones) {
                ...
                for (var i in resultOperaciones) {
                    ...
                    if (resultOperaciones[i][8] != null)
                        props.atos_numerodetecnicos = Math.floor(resultOperaciones[i][8]);
                    if (resultOperaciones[i][9] != null)
                        props.atos_trabajo = resultOperaciones[i][9];
                    if (resultOperaciones[i][10] != null)
                        props.atos_porcentajetrabajo = Math.floor(resultOperaciones[i][10]);
                    ...
                    if (entityForm.entity.id != null)
                        props.atos_hojaderutaordendetrabajoid = new MobileCRM.Reference("atos_hojaderutadeordendetrabajo", entityForm.entity.id, "");
                    ...
                    if (resultOperaciones[i][15] != null) {
                        ...
                        newOperacionOT.save(
                            function (err) {
                                ...
                                if (err) {
                                    MobileCRM.UI.MessageBox.sayText('error al crear operacion');
                                    MobileCRM.UI.MessageBox.sayText(err);
                                    //saveHandler.resumeSave();
                                    guardar = true;
                                } else {
                                    MobileCRM.UI.MessageBox.sayText('operacion creada');
                                    var ultimaOperacion = false;
                                    if (operacionInsertada == resultOperaciones.length)
                                        ultimaOperacion = true;
                                    FS.HojaRuta.crearServiciosRutaOT(entityForm, orden, hojaRutaId, resultOperaciones[operacionInsertada - 1], this, ubicacion, ultimaOperacion, tipoMovimiento, HojaRuta);
                                }
                            });
                    } else {
                ...
                if (resultOperaciones.length == 0) {
                    // TERMINA
                    saveHandler.resumePostSave().then(function () {
                        FS.HojaRuta.saveAndCloseForm();
                    });
                    //MobileCRM.UI.EntityForm.saveAndClose();
                    //FS.HojaRuta.saveAndCloseForm();
                }
    ...
    crearServiciosRutaOT: function (entityForm, orden, hojaRutaId, operacionPlantilla, operacion, ubicacion, ultima, tipoMovimiento, HojaRuta) {
                        ...
                        function (err) {
                            ...
                            if (err) {
                                ...
                                if (servicioInsertado == resultServicios.length)
                                    FS.HojaRuta.crearComponentesRutaOT(entityForm, orden, hojaRutaId, operacionPlantilla, operacion, ubicacion, ultimaOperacion, tipoMovimiento, HojaRuta);
                            }
                        });
                }
                // si no tiene operaciones cierro el proceso
                if (resultServicios.length == 0) {
                    FS.HojaRuta.crearComponentesRutaOT(entityForm, orden, hojaRutaId, operacionPlantilla, operacion, ubicacion, ultima, tipoMovimiento, HojaRuta);
                }
            },
            function (err) {
                FS.CommonEDPR.GetErrorCollectionByCode('JS_001', entityForm);
    ...
    crearComponentesRutaOT: function (entityForm, orden, hojaRutaId, operacionPlantilla, operacion, ubicacion, ultima, tipoMovimiento, HojaRuta) {
        ...
        var fetchXmlComponente = "<fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>" +
            ...
            "    <order attribute='atos_numerosap' descending='false'/>" +
            "    <filter type='and'>" +
            "      <condition attribute='atos_operaciondehojaderutaid' operator='eq' value='" + operacionPlantillaId + "' />" +
            "      <condition attribute='atos_flagdeborrado' operator='eq' value='0' />" +
            "    </filter>" +
        ...
        MobileCRM.FetchXml.Fetch.executeFromXML(fetchXmlComponente,
            function (resultComponentes) {
                ...
                for (var i in resultComponentes) {
                    ...
                    props.atos_hojarutaorigen = props.atos_hojarutaorigen = new MobileCRM.Reference("atos_hojaderuta", HojaRutaPlantilla[0], "");
                    props.atos_origendetransaccion = transaccionHojaRuta;
                    // decimos que la creacion se ha realizado desde el movil para que no se dupliquen los campos
                    props.atos_origen = 300000002;
    ...
    DesactivarOperacionesOT: function (entityForm, orden, HojaRuta) {
        ...
        MobileCRM.FetchXml.Fetch.executeFromXML(fetchXmlOperacionesBorrado,
            function (resultOperacionesBorrado) {
                    ...
                    operacionOT.save(
                        function (err) {
                            ...
                            if (err) {
                                ...
                                FS.HojaRuta.DesactivarServiciosOT(entityForm, this.id, orden, HojaRuta);
                ...
                if (resultOperacionesBorrado.length == 0) {
                    ...
                    FS.HojaRuta.crearDatosHojaRutaOT(entityForm, orden, entityForm.entity.properties.atos_hojaderutaid.id, HojaRuta);
                }
    ...
    DesactivarServiciosOT: function (entityForm, operacionId, orden, HojaRuta) {
                        ...
                        function (err) {
                            ...
                            if (err) {
                                ...
                                if (servicioBorradoInsertado == resultServiciosBorrado.length)
                                    FS.HojaRuta.DesactivarComponetesOT(entityForm, operacionId, orden, HojaRuta);
                            }
                        });
                }
    ...
    DesactivarComponetesOT: function (entityForm, operacionId, orden, HojaRuta) {
