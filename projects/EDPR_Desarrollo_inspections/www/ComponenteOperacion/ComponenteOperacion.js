...
if (typeof ComponenteOperacion == "undefined") {
  ComponenteOperacion = { __namespace: true };
}
...
var isComponentNew_UpdateWO_setPartsUsed = false;
var popupNoDeleteComponent = false;
...
FS.ComponenteOperacion = {
  ...
  onLoad: function (entityForm) {
    ...
    if (componente.isNew) {
      ...
      if (entityForm.entity.properties.atos_operacionid != null) {
        ...
        var varFetchXml =
          ...
          "       <filter type='and'>" +
          "           <condition attribute='msdyn_workorderservicetaskid' value='" +
          operacionId +
          " ' operator='eq'/> " +
          "       </filter>  " +
          " </entity>  " +
        ...
        MobileCRM.FetchXml.Fetch.executeFromXML(
          ...
          function (result) {
            for (var i in result) {
              MobileCRM.UI.EntityForm.requestObject(
                function (entityForm) {
                  entityForm.entity.properties.atos_ordendetrabajoid =
                    new MobileCRM.Reference(
                      "msdyn_workorder",
                      result[i][0].id,
                      result[i][0].primaryName
                    );
                  self.completarCentroplanificacion(entityForm);
                  ///AAC-2020-05-11
                  /// REDMMINE: 21808
    ...
    //5785
    FS.ComponenteOperacion.checkComponentToDelete(entityForm);
    //5785 end
  },
  ...
  onSave: function (entityForm) {
    ...
    isComponentNew_UpdateWO_setPartsUsed = entityForm.entity.isNew;
  },
  ...
  onPostSave: function (entityForm) {
    ...
    try {
      if (enviarOT_A_SAP) {
        ...
        FS.ComponenteOperacion.actualizarOTEnvioSAP(
          ordenTrabajo.id,
          entityForm
        );
      }
    ...
    self.updateWOSetPartsUsed(entityForm);

    //MobileCRM.bridge.closeForm();
  },

  //MF: MM-3674 || Fixed
  updateWOSetPartsUsed: function (entityForm) {
    try {
      var postSuspend = entityForm.suspendPostSave();
      var workOrderID = entityForm.entity.properties.atos_ordendetrabajoid.id;
      var itemCategory = entityForm.entity.properties.atos_listadematerial;
      if (
        itemCategory !== null &&
        itemCategory !== undefined &&
        itemCategory !== ItemCategory.T_NoPartsUsed &&
        isComponentNew_UpdateWO_setPartsUsed === true
      ) {
        var workOrder = new MobileCRM.DynamicEntity(
          "msdyn_workorder",
          workOrderID
        );
        workOrder.isNew = false;
        workOrder.properties.atos_nopartsused = true;
        workOrder.save(function (err) {
          if (err) {
            MobileCRM.UI.MessageBox.sayText(err);
          } else {
            //MobileCRM.UI.MessageBox.sayText("WO Parted Used updated!");
          }
          postSuspend.resumePostSave();
        }); //save
      }

      //var countComponentsFetchXml = "<fetch aggregate='true'> " +
      //                              "	<entity name='atos_componenteoperacion'> " +
      //                              "  <attribute name='atos_componenteoperacionid' alias='total' aggregate='count' /> " +
      //                              "	    <filter> " +
      ...
      //MobileCRM.FetchXml.Fetch.executeFromXML(countComponentsFetchXml,
      //    function (result) {
      //        var count = result[0][0];
      //        if (count !== null && count !== undefined && count > 0) {
      //            if (isComponentNew_UpdateWO_setPartsUsed === true) {
  ...
  ActivarConfirmacion: function (entityForm, confirmarActivo) {
    ...
    dv2.getItemByName("atos_documentodeconfirmacion").isEnabled =
      confirmarActivo;
    dv2.getItemByName("atos_aniodocumentoconfirmacion").isEnabled =
      confirmarActivo;
    dv2.getItemByName("atos_documentodecancelacion").isEnabled =
      !confirmarActivo;
  },
    ...
    if (entityForm.entity.properties.atos_ordendetrabajoid != null) {
      ...
      MobileCRM.FetchXml.Fetch.executeFromXML(
        ...
        function (result) {
          ...
          if (
            noPartUsed == "True" &&
            FS.ComponenteOperacion.EsUbicacionAmericana(UTname) == true
          ) {
            sinComponentes = true;
          }
        },
  ...
  onChange: function (entityForm) {
    ...
    switch (changedItem) {
      ...
      case "atos_materialid":
        if (!aprobado) self.OnChangeMateriales(entityForm);
        break;
      case "atos_cantidad":
        if (!aprobado) self.onChangeCantidad(entityForm);
        break;
      case "atos_cantidadadconfirmacion":
        if (!aprobado) self.onChangeCantidadConfirmacion(entityForm);
        break;
      case "atos_centrocomponenteid":
        if (!aprobado) self.onChangeCentro(entityForm);
        break;
      case "atos_almacenid":
        if (!aprobado) self.onChangeAlmacen(entityForm);
        break;
    }
  ...
  onChangeCantidad: function (entityForm) {
      ...
      MobileCRM.UI.EntityForm.requestObject(
        function (entityForm) {
          if (entityForm.entity.properties.atos_cantidad != null) {
            entityForm.entity.properties.atos_cantidadadconfirmacion =
              entityForm.entity.properties.atos_cantidad;
          } else {
    ...
    if (unidadMedidaId != null) {
      var fetchXml =
        ...
        "<filter type='and'>" +
        "<condition attribute='atos_unidaddemedidaid' operator='eq' value='" +
        unidadMedidaId +
        "' />" +
        "</filter>" +
        "</entity>" +
      ...
      MobileCRM.FetchXml.Fetch.executeFromXML(
        ...
        function (result) {
          ...
          for (var i in result) {
            ...
            if (
              numDecimales != null &&
              entityForm.entity.properties.atos_cantidad != null
            ) {
      ...
      function (entityForm) {
        if (entityForm.entity.properties.atos_centrocomponenteid != null) {
          entityForm.entity.properties.atos_centroid =
            entityForm.entity.properties.atos_centrocomponenteid;
        } else {
    ...
    MobileCRM.UI.EntityForm.requestObject(
      function (entityForm) {
        if (entityForm.entity.properties.atos_almacenid != null) {
          entityForm.entity.properties.atos_almacenconfirmacionid =
            entityForm.entity.properties.atos_almacenid;
        } else {
  ...
  CamposPorDefectoMaterial: function (entityForm) {
    if (entityForm.entity.properties.atos_materialid != null) {
      ...
      var varFetchXml =
        ...
        "       <filter type='and'>" +
        "           <condition attribute='productid' value='" +
        materialId +
        " ' operator='eq'/> " +
        "       </filter>  " +
        " </entity>  " +
      ...
      MobileCRM.FetchXml.Fetch.executeFromXML(
        ...
        function (result) {
          for (var i in result) {
            MobileCRM.UI.EntityForm.requestObject(
              function (entityForm) {
                entityForm.entity.properties.atos_unidademedidaid =
                  result[i][0];
                entityForm.entity.properties.atos_grupodearticulosid =
                  result[i][1];
                // confirmacion
                entityForm.entity.properties.atos_unidadconfirmacionid =
                  entityForm.entity.properties.atos_unidademedidaid;
              },
              FS.CommonEDPR.onError,
              null
  ...
  SetValorGrupoCompras: function (entityForm) {
    ...
    var centroPlanificacion =
      entityForm.entity.properties.atos_centrodeplanificacionid;
    if (materialValue != null && centroPlanificacion != null) {
      ...
      var varFetchXml =
        ...
        "<filter type='and'>" +
        "<condition attribute='atos_materialid' value='" +
        materialValueId +
        "' operator='eq'/>" +
        "</filter>" +
        ...
        "<filter type='and'>" +
        "<condition attribute='atos_centrodeplanificacionid' value='" +
        centroPlanificacionId +
        "' uitype='atos_centrodeplanificacion' uiname='100A: EDP Renewables N. America LLC' operator='eq'/>" +
        "</filter>" +
        "</link-entity>" +
        "</link-entity>" +
        "</link-entity>" +
      ...
      MobileCRM.FetchXml.Fetch.executeFromXML(
        ...
        function (result) {
          for (var i in result) {
            MobileCRM.UI.EntityForm.requestObject(
              function (entityForm) {
                entityForm.entity.properties.atos_grupodecomprasid =
                  new MobileCRM.Reference(
                    "atos_grupodecompras",
                    resul[i][2],
                    resul[i][0]
      ...
      MobileCRM.FetchXml.Fetch.executeFromXML(
        ...
        function (result1) {
          ...
          var varFetchXmlUbicacion =
            ...
            "      <filter type='and'>" +
            "        <condition attribute='msdyn_workorderservicetaskid' operator='eq' uitype='msdyn_workorderservicetask' value='" +
            operacionId +
            "' />" +
            "      </filter>" +
            "    </link-entity>" +
            ...
            function (result) {
              if (result.length > 0) {
                ...
                if (hrorigen != null && hrorigen != "") {
                  filterHR +=
                    "<condition attribute='atos_indicadorboq' operator='eq' value='1'/>";
                } else {
                  filterHR +=
                    "<condition attribute='atos_indicadorboq' operator='ne' value='1'/>";
                }

                if (result1[0][1] != null) {
                  filterPlanif +=
                    "<condition attribute='atos_centroid' operator='eq' value='" +
                    centroId +
                    "' />";
                }
                if (ubicacion[1] != null) {
                  filterCoste +=
                    "<condition attribute='atos_centrodecosteid' operator='eq' value='" +
                    ubicacion[1].id +
                    "'/>";
                }
                      ...
                      if (almacen[2] != null) {
                        MobileCRM.UI.EntityForm.requestObject(
                          function (entityForm) {
                            entityForm.entity.properties.atos_centrocomponenteid =
                              new MobileCRM.Reference(
                                almacen[2].entityName,
                                almacen[2].id,
                                almacen[2].primaryName
                              );
                            entityForm.entity.properties.atos_centroid =
                              entityForm.entity.properties.atos_centrocomponenteid;
                          },
                      ...
                      if (almacen[1] != null) {
                        MobileCRM.UI.EntityForm.requestObject(
                          function (entityForm) {
                            entityForm.entity.properties.atos_almacenid =
                              new MobileCRM.Reference(
                                almacen[1].entityName,
                                almacen[1].id,
                                almacen[1].primaryName
                              );
                            entityForm.entity.properties.atos_almacenconfirmadoid =
                              entityForm.entity.properties.atos_almacenid;
                          },
                          FS.CommonEDPR.onError,
                          null
      ...
      MobileCRM.FetchXml.Fetch.executeFromXML(
        ...
        function (result) {
          for (var i in result) {
            MobileCRM.UI.EntityForm.requestObject(
              function (entityForm) {
                entityForm.entity.properties.atos_tipodemovimientoid =
                  new MobileCRM.Reference(
                    "atos_tipodemovimiento",
                    result[i][0],
                    result[i][1]
                  );
              },
  ...
  completarUTyLiberada: function (entityForm) {
    ...
    if (entityForm.entity.properties.atos_ordendetrabajoid != null) {
      ...
      MobileCRM.FetchXml.Fetch.executeFromXML(
        ...
        function (result) {
          for (var i in result) {
            if (result[i][1] != null) UTname = result[i][1].primaryName;
            if (result[i][1] != null) liberada = result[i][0];
          }
        },
        function (err) {
          FS.CommonEDPR.GetErrorCollectionByCode("JS_001");
  ...
  tienePermisosJefeParque: function (entityForm, tipo) {
    ...
    MobileCRM.FetchXml.Fetch.executeFromXML(
      ...
      function (result) {
        if (result.length > 0) {
          if (tipo == 1) {
            FS.ComponenteOperacion.IntegrarConfirmacionComponentesConSAP(
              entityForm
            );
          }
          if (tipo == 2) {
            FS.ComponenteOperacion.IntegrarCancelacionComponentesConSAP(
              entityForm
            );
          }
        } else {
      ...
      function success(result) {
        ...
        popup.show(function (button) {
          if (button == "Yes" || button == "Si") {
            ...
            debugger;
            //MM-5785 not allow delete of an operation if have a TO associated and it is success integrated checkOperationToDelete()
            if (popupNoDeleteComponent == true) {

              MobileCRM.UI.MessageBox.sayText("Unable to delete one or more component lines due to Transfer Order.");
              stop = true
            }
  ...
  estaAutorizada: function (resultadoAut) {
    ...
    for (var i in resultadoAut) {
      if (
        resultadoAut[i][5] == codigoPermisoAutorizado &&
        resultadoAut[i][4] != null
      ) {
        esAutorizada = true;
      }
    }
  ...
  IntegrarConfirmacionComponentesConSAP: function (entityForm) {
    ...
    var validacionOK = ValidarCamposObligatoriosComponente(
      IdiomaUsuario,
      entityForm
    );
    if (validacionOK != "") {
    ...
    MobileCRM.FetchXml.Fetch.executeFromXML(
      ...
      function success(resultOT) {
        ...
        var fetchAutoriz = ObtenerFetchDeMensajeAutorizacionesOT(
          ordenTrabajo.id,
          activo
        );
                  ...
                  popup.show(function (button) {
                    if (button == "Yes" || button == "Si") {
                      ...
                      MobileCRM.UI.EntityForm.requestObject(
                        function (entityForm) {
                          entityForm.entity.properties.atos_estatusconfirmacion = 300000001;
                          FS.ComponenteOperacion.ActivarConfirmacion(
                            entityForm,
                            false
                          );
                        },
  ...
  checkComponentToDelete: function (entityForm) {
    //MM-5785
    if (entityForm.entity.properties.atos_ordendetrabajoid != null) {
      var WOId = entityForm.entity.properties.atos_ordendetrabajoid.id;
      var materialId = entityForm.entity.properties.atos_materialid.id;
      ...
      var fetchXml = `
        <fetch>
          <entity name="msdyn_workorder">
            <attribute name="msdyn_serviceaccount" />
            <filter>
              <condition attribute="msdyn_workorderid" operator="eq" value="${WOId}" />
            </filter>
          </entity>
        </fetch>`;
      MobileCRM.FetchXml.Fetch.executeFromXML(fetchXml,
        function success(resultWO) {
          debugger;
          if (resultWO != null && resultWO[0].length > 0) {
            var esUbicacionAmericana = FS.ComponenteOperacion.EsUbicacionAmericana(resultWO[0][0].primaryName);
              ...
              var fetchXmlTO = `
                            <fetch>
                                ...
                                <link-entity name="edprdyn_transferorderitem" from="edprdyn_transferorderid" to="edprdyn_transferorderid" alias="TO">
                                    ...
                                    <filter>
                                    <condition attribute="edprdyn_product" operator="eq" value="${materialId}" />
                                    </filter>
                                </link-entity>
                                </entity>
                            </fetch>`;
              MobileCRM.FetchXml.Fetch.executeFromXML(fetchXmlTO,
                function success(resultTO) {
                  debugger;
                  if (resultTO != null && resultTO.length > 0) {
                    popupNoDeleteComponent = true;
                  }
                },
                function (error) {
                  console.error("Error getting transferorder", error);
                }
              );
            }
          }
  ...
  IntegrarCancelacionComponentesConSAP: function (entityForm) {
    ...
    var validacionOK = ValidarCamposObligatoriosComponenteCancelacion(
      IdiomaUsuario,
      entityForm
    );
    if (validacionOK != "") {
...
function Literales(idioma) {
  if (idioma != Idioma.ingles) {
    return {
      ...
      ListadoDeMateriales:
        "*Tipo de posision no es: L en lista, quite esta operación*",
    };
  } else {
    return {
...
function ObtenerFetchDeMensajeAutorizaciones(
  idOrdenTrabajo,
  activo,
...
function FetchDeMensajePendienteIntegrar(idOrdenTrabajo) {
  var fetchXml =
    ...
    "<filter type='and'>" +
    "<condition attribute='atos_guidobjeto' operator='eq' value='" +
    idOrdenTrabajo +
    "' />" +
    "</filter>" +
    "</entity>" +
...
function ValidarCamposObligatoriosComponente(IdiomaFormulario, entityForm) {
  ...
  try {
    ...
    if (entityForm.entity.properties.atos_operacionid == null) {
      if (respuesta == "")
        if (IdiomaFormulario == 3082) respuesta = "Operación";
        else respuesta = "Operation";
      else if (IdiomaFormulario == 3082) respuesta = respuesta + ", Operación";
      else respuesta = respuesta + ", Operation";
    }

    if (entityForm.entity.properties.atos_materialid == null) {
      if (respuesta == "")
        if (IdiomaFormulario == 3082) respuesta = "Material";
        else respuesta = "Material";
      else if (IdiomaFormulario == 3082) respuesta = respuesta + ", Material";
      else respuesta = respuesta + ", Material";
    }

    if (entityForm.entity.properties.atos_posicion == null) {
      if (respuesta == "")
        if (IdiomaFormulario == 3082) respuesta = "Posición";
        else respuesta = "Position";
      else if (IdiomaFormulario == 3082) respuesta = respuesta + ", Posición";
      else respuesta = respuesta + ", Position";
    }

    if (entityForm.entity.properties.atos_cantidad == null) {
      if (respuesta == "")
        if (IdiomaFormulario == 3082) respuesta = "Cantidad";
        else respuesta = "Quantity";
      else if (IdiomaFormulario == 3082) respuesta = respuesta + ", Cantidad";
      else respuesta = respuesta + ", Quantity";
    }

    if (entityForm.entity.properties.atos_unidademedidaid == null) {
      if (respuesta == "")
        if (IdiomaFormulario == 3082) respuesta = "Unidad de medida";
        else respuesta = "Unity of measure";
      else if (IdiomaFormulario == 3082)
        respuesta = respuesta + ", Unidad de medida";
      else respuesta = respuesta + ", Unity of measure";
    }

    if (entityForm.entity.properties.atos_centroid == null) {
      if (respuesta == "")
        if (IdiomaFormulario == 3082) respuesta = "Centro confirmación";
        else respuesta = "Center confirmation";
      else if (IdiomaFormulario == 3082)
        respuesta = respuesta + ", Centro confirmación";
      else respuesta = respuesta + ", Center confirmation";
    }

    if (entityForm.entity.properties.atos_almacenid == null) {
      if (respuesta == "")
        if (IdiomaFormulario == 3082) respuesta = "Almacén";
        else respuesta = "Storage location";
      else if (IdiomaFormulario == 3082) respuesta = respuesta + ", Almacén";
      else respuesta = respuesta + ", Storage location";
    }
    ...
    if (entityForm.entity.properties.atos_listadematerial == null) {
      if (respuesta == "")
        if (IdiomaFormulario == 3082) respuesta = "Tipo de posición";
        else respuesta = "Item category";
      else if (IdiomaFormulario == 3082)
        respuesta = respuesta + ", Tipo de posición";
      else respuesta = respuesta + ", Item category";
    }

    if (
      entityForm.entity.properties.atos_cantidadadconfirmacion == null ||
      entityForm.entity.properties.atos_cantidadadconfirmacion < 0
    ) {
      if (respuesta == "")
        if (IdiomaFormulario == 3082)
          respuesta = "Cantidad confirmación (debe ser mayor que cero)";
        else respuesta = "Quantity confirmation (must to be greater than cero)";
      else if (IdiomaFormulario == 3082)
        respuesta = respuesta + ", Cantidad confirmación";
      else respuesta = respuesta + ", Quantity confirmation";
    }
...
function ValidarCamposObligatoriosComponenteCancelacion(
  IdiomaFormulario,
  entityForm
) {
  var respuesta = "";
  try {
    ...
    if (entityForm.entity.properties.atos_fechacontabilidad == null) {
      if (respuesta == "")
        if (IdiomaFormulario == 3082) respuesta = "Fecha contabilidad conf.";
        else respuesta = "ConfirmationAccounting date";
      else if (IdiomaFormulario == 3082)
        respuesta = respuesta + ", Fecha contabilidad conf.";
      else respuesta = respuesta + ", ConfirmationAccounting date";
    }

    if (entityForm.entity.properties.atos_documentodeconfirmacion == null) {
      if (respuesta == "")
        if (IdiomaFormulario == 3082) respuesta = "Documento de confirmación";
        else respuesta = "Document confirmation";
      else if (IdiomaFormulario == 3082)
        respuesta = respuesta + ", Documento de confirmación";
      else respuesta = respuesta + ", Document confirmation";
    }
    ...
    if (entityForm.entity.properties.atos_aniodocumentoconfirmacion == null) {
      if (respuesta == "")
        ...
        else respuesta = "Year of the confirmation document";
      else if (IdiomaFormulario == 3082)
        respuesta = respuesta + ", Año del documento de Confirmación";
      else respuesta = respuesta + ", Year of the confirmation document";
    }

    if (entityForm.entity.properties.atos_flagcancelado == null) {
      if (respuesta == "")
        if (IdiomaFormulario == 3082) respuesta = "Cancelado";
        else respuesta = "Cancelled";
      else if (IdiomaFormulario == 3082) respuesta = respuesta + ", Cancelado";
      else respuesta = respuesta + ", Cancelled";
    }

    if (
      entityForm.entity.properties.atos_estatusconfirmacion == null ||
      entityForm.entity.properties.atos_estatusconfirmacion != confirmado
    ) {
      if (respuesta == "")
        if (IdiomaFormulario == 3082)
          respuesta = "--El componente no está confirmado-- ";
        else respuesta = "--The component isn't confirmed-- ";
      else if (IdiomaFormulario == 3082)
        respuesta = respuesta + " *--El componente no está confirmado--*";
      else respuesta = respuesta + " *--The component isn't confirmed--*";
    }
