...
FS.OrdenTrabajo = {
    ...
    onPostSave: function (entityForm) {
		...
		self.onSaveNewOT(entityForm.entity);
										...
										popup.show(
											function (button) {
												if (button == "Yes" || button == "Si") {
													...
													self.CierreAvisoConParada(entityForm);		
												}
												else {
													MobileCRM.UI.EntityForm.requestObject(
														function (entityForm) {
			...
			if (FS.OrdenTrabajo.EsUbicacionAmericana(UTname)) {
				...
				MobileCRM.FetchXml.Fetch.executeFromXML(fetchXml,
					function (result) {
						...
						if (result.length == 0){
							if (IdiomaUsuario == Idioma.ingles)	{
								NoAdjuntosNoComments = NoAdjuntosNoComments + "Documents missing in the attachments. "; 
							}
							else {
								NoAdjuntosNoComments = NoAdjuntosNoComments + "La OT no tiene documentos adjuntos. "; 
							}
						}
						...
						if (entityForm.entity.properties.msdyn_primaryincidentdescription == null || entityForm.entity.properties.msdyn_primaryincidentdescription == ""){
							if (IdiomaUsuario == Idioma.ingles)	{
								NoAdjuntosNoComments = NoAdjuntosNoComments + "Comments missing in the long text section. "; 
							}
							else {
								NoAdjuntosNoComments = NoAdjuntosNoComments + "La OT no tiene descripción larga. "; 
							}
						}
...
//21935 : IMPORTANTE ESTE CAMBVIO GUARDA DOS VECES PARA QUE SE LLEVE EL CAMBIO DE ORIGEN A CRM
		    ...
		    MobileCRM.FetchXml.Fetch.executeFromXML(fetchXml,
                function (result) {
					...
					for (var i in result) {
						...
						props.atos_fechadelcomponente = result[i][9];
            ...
            MobileCRM.FetchXml.Fetch.executeFromXML(fetchXml,
                function (result) {
					if (result.length > 0) {
						...
						props.atos_listadematerial =300000000;
 ...
 ComprobarAvisoNoProgramable: function (entityForm) {
    ...
    ComprobarFechasEquipo: function () {
        try {
            if (entityForm.entity.properties.msdyn_customerasset!= null)
             	var equipoId =  entityForm.entity.properties.msdyn_customerasset.id;
