...
FS.ObjetoOrdenTrabajo = {
    ...
    onSave: function (entityForm) {
        var self = this;
        // si tiene aviso le asigno al aviso la orden de trabajo
		if (entityForm.entity.properties.atos_avisoid!=null){
			self.ActualizarAviso(entityForm);	
		}
    },
	
	// Guarda el aviso
	ActualizarAviso: function (entityForm) {
		
		var Aviso = new MobileCRM.DynamicEntity.createNew("atos_aviso");
		Aviso.id =  entityForm.entity.properties.atos_avisoid.id;
		Aviso.isNew = false;
		Aviso.properties.atos_origen = 1;
		...
		Aviso.id =  entityForm.entity.properties.atos_avisoid.id;
		Aviso.isNew = false;
		Aviso.properties.atos_ordendetrabajoid = new MobileCRM.Reference("msdyn_workorder", entityForm.entity.properties.atos_ordendetrabajoid.id , "");
		Aviso.properties.atos_origen = 300000002;
				...
				function (button) {
					if (button == "Yes" || button == "Si") {
						MobileCRM.UI.EntityForm.requestObject(
							...
							entityForm.entity.properties.atos_indicadordeborrado = true;
