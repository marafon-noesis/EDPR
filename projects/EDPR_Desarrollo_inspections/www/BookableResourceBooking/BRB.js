...
FS.BookableResourceBooking = {
        ...
        if (changedItem && editedEntity && editedEntity.properties) {
            switch (changedItem) {
                ...
                case FS.Schema.BookableResourceBooking.properties.endTime:
                    FS.BookableResourceBooking.updateDuration(editedEntity,entityForm);
                    break;
    ...
    updateDuration: function (editedEntity,entityForm) {
    ...
    handleSave: function (entityForm) {
        ...
        if (errorMessage) {
		 ...
		 if (!!entity.properties[FS.Schema.BookableResourceBooking.properties.startTime] &&
            !!entity.properties[FS.Schema.BookableResourceBooking.properties.endTime]) {
			// REDMINE:213248 AAC 2021-09-20 Validaciones en entidad reservas de recursos que se pueden reservar	
			var fechaInicio = entity.properties[FS.Schema.BookableResourceBooking.properties.startTime];	
			var fechaFin = entity.properties[FS.Schema.BookableResourceBooking.properties.endTime];	
			var yearFI = fechaInicio.getFullYear() + "";
            var monthFI = (fechaInicio.getMonth() + 1) + "";
            var dayFI = fechaInicio.getDate() + "";
            var dateFormatfechaInicio = yearFI + "-" + monthFI + "-" + dayFI;
            var yearFF = fechaFin.getFullYear() + "";
            var monthFF = (fechaFin.getMonth() + 1) + "";
            var dayFF = fechaFin.getDate() + "";
            var dateFormatfechaFin = yearFF + "-" + monthFF + "-" + dayFF;
			
			if (dateFormatfechaInicio != dateFormatfechaFin){
                 entityForm.context.errorMessage = FS.BookableResourceBooking.localization.get("Alert.BookingStatus_FechaFin");
				// Return true to apply changed values
				return true;
			}
        }
